...
| 2-row × 5-col ERA5 vs CMIP6 seasonal comparison | `(~14, ~15)` inches (auto-computed in function) |

Use `visualization.make_spatial_figure()` to compute sizes automatically.

### 1.7 Seasonal comparison maps

When comparing ERA5-Land and CMIP6 seasonal climatologies, use the
**2-row × 5-column** layout (not the old 4×2 layout).

```
Row 0: [ERA5 DJF] [CMIP DJF] [narrow sep] [ERA5 MAM] [CMIP MAM]
Row 1: [ERA5 JJA] [CMIP JJA] [narrow sep] [ERA5 SON] [CMIP SON]
Row 2: [horizontal colorbar spanning full width]
```
...
fig = viz.plot_seasonal_comparison_maps(
    ...
    cmip_lats, cmip_lons,
    region=REGION,
    show_coarse_grid=True,
    highlight_box=dict(lat_min=32, lat_max=34, lon_min=34, lon_max=36,
                       label='Largest bias:\nCold terrain\nundersampled\nby coarse grid'),
...
Key layout details:
- Column 2 (index) is a narrow separator (`width_ratio ≈ 0.10` of a map column).
- GridSpec `hspace=0.30`, `wspace=0.14` for visible row/column padding.
- Shared horizontal colorbar at the bottom via `fig.add_axes([...])`.
- Coarse-grid overlay on ERA5 (left) panels only.
- `highlight_box` draws a bold red rectangle on **all 8 panels**; the annotation
  label appears on the DJF-CMIP panel (row 0, col 1) only to avoid clutter.
- Colorbar ticks formatted with `_temp_formatter` (`20°`, `25°`, etc.).

### 1.6 Highlight boxes on spatial maps

To draw attention to a geographic sub-region, pass a `highlight_box` dict to
any spatial plot function that accepts it:

```python
highlight_box = dict(
    lat_min=32, lat_max=34,
    lon_min=34, lon_max=36,
    label="Short annotation\ntext",   # optional; None to suppress
)
```

This draws a bold red rectangle via `_draw_highlight_box`. The annotation
label is rendered as a white-backed red-bordered box just outside the
top-right corner of the rectangle.

**Current usage:** the region 32–34°N, 34–36°E (≈ 4 CMIP6 cells) is
highlighted in Fig 2 and Fig 4 to mark the area of largest ERA5–CMIP6
climatological bias, where cold mountainous terrain is undersampled by the
coarse grid. Draw the annotation label on one panel only to avoid clutter.
...
### 2.3 Confidence bands on time series

All climatology and aggregated time series plots must include a shaded
confidence band. The correct interpretation of the band depends on the
aggregation level:

| Figure | Band type | Implementation |
|---|---|---|
| Monthly climatology (fig03b) | 90% t-CI on the **inter-annual** mean per calendar month | `plot_monthly_climatology` computes automatically; n = 10 yr |
| Monthly/annual aggregated trend (fig03, fig03c) | ±1 SD of daily values within each period | Pass `fill_dict` to `plot_domain_timeseries` |

**Legend requirement:** every plot with a CI band must include a legend entry
(grey patch) explaining what the band represents. Both functions add this
automatically. The legend entry text format:

- Monthly climatology: `"Shaded: 90% CI on inter-annual mean (±t·SE, n=10 yr)"`
- Domain timeseries with `fill_dict`: `"Shaded: ±1 SD of daily values within each period"`

**Do not** add CI bands to quarterly warming trend plots (fig10) — with only
10 annual data points and a single value per date, the band has no statistical
meaning. Use `show_trend_band=False`.

**Transparency:** `CI_ALPHA = 0.27` (set in `vis_constants.py`). The rendered
legend proxy uses `min(CI_ALPHA * 1.8, 0.7)` for legibility.

### 2.4 Multi-series styling
